...
- record identity: suite, harness, environment, entry, workload, scale, metric, boundary, unit,
  plus nullable contract version / commit policy dimensions. In schema v3 every Web record stores
  a structured execution regime in `environment`: `jsRegime`, exact `jsFlags`, and
  `cpuThrottle`; Native keeps its environment string and cannot carry Web regime fields;
