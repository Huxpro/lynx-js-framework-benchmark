...
- Native campaign identity: versioned complete-matrix hash (23 cells per current entry), immutable input-receipt and connector
...
- the Native complete-cohort coverage classification and totals. Each cell is derived as measured,
...
7. A publishable Native checkpoint materializes exactly 23 cells per current per-harness featured
   entry (184 total for the eight-entry M3 cohort).
