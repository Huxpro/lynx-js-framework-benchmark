...
- **Native interactive latency** = the entry's Native input handler → its second
  ...
  for every Native-eligible featured entry. Upstream Octane and its archived PR snapshot remain
  Web-only. The Hux #269 + #272 composite includes reviewed benchmark-app instrumentation whose
  exact patch is retained in provenance; it reports the real renderer acknowledgement and second
  Native frame without patching Octane runtime code.
...
- **Native startup** = Lynx pipeline `openTime` → `totalFcp.duration` from
  ...
  Octane's custom renderer does not expose that pipeline boundary in this Explorer build. The Hux
  composite therefore reports separate `octaneCommitAck` and `octaneSecondFrame` startup metrics,
  with explicit renderer-acknowledgement boundaries; they are never relabelled as FCP or settled.
...
- **List virtualization (capability-gated)**: `list-startup@1k/10k` attaches a separately built,
  prepopulated declarative list and observes the first visible content tree. `list-recycle@10k`
  moves exactly one 390×640 viewport twenty times and retains raw elapsed time, recycled-cell count,
  and wire totals. Web `list-fling@10k` applies 4,800 px/s for 1,500 ms. Native performs a 600 px
  touch gesture whose release velocity is 4,800 px/s, then observes for 1,500 ms after release.
  Both retain materialized cells and every post-release blank observer frame; Web additionally
  retains materialization samples. Collection alone derives cost/bytes per recycled
  ...
  tree and uses a shared release-velocity touch gesture; Web uses its separate composed-tree observer
  and continuous wheel schedule. A materialization is a stable item key first becoming visible in
  an observer frame; a blank observer frame has zero expected visible keys. Native DevTool exposes
  neither a framework-neutral transport byte counter nor a common scroll-offset
  presentation clock, so Native wire bytes and expected-entry materialization latency are explicit
  metric-scoped unsupported cells rather than estimates. Renderer internals such as
...
- Web table and pipeline sampling opens a fresh page for each formal repetition and performs two
  untimed create/clear cycles on that page before preparing the measured state. This prevents
  cumulative app state from crossing repetition boundaries while keeping the same lifecycle for
  every featured entry. Startup likewise uses a fresh page per sample (page-load variance is
  inside the sample, stated on the site). Native opens a fresh
...
- Upstream Octane and its archived PR snapshot are explicitly Web-only. The provenance-pinned Hux
  #269 + #272 composite is the sole featured Octane Native producer; historical private-protocol
  Octane observations remain appendix evidence and never enter the current Native cohort.
