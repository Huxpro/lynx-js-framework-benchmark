...
7. Table and pipeline use a fresh page per formal rep, run two unmeasured create/clear warmups on
   that page, prepare the required prestate, measure once, and close the page. Startup also uses a
   fresh page per sample. No cumulative app state crosses a repetition boundary.
...
identity. Upstream/archived Octane and Lab variants are not scheduled on Native; only the reviewed
Hux composite producer joins the featured Native matrix.
...
Explorer build. The Hux composite therefore keeps renderer-acknowledgement and second-frame
startup metrics under Octane-specific names instead of presenting them as FCP or settled; older
private-protocol observations remain historical appendix evidence only.
