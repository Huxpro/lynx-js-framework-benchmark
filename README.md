...
| staging Pareto | exact scale-matched artifact/MTS gzip × startup FCP median and CI | collector-derived static artifact records + run observations; lower-left frontier only, never a score |
...
then-current per-scale artifact bytes do not borrow today's manifest.
