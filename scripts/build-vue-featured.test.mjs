...
const script = new URL('./build-vue-featured.mjs', import.meta.url).pathname;
...
test('M0 profile emits separate default and optimized comparator matrices', () => {
  const checkout = fs.mkdtempSync(path.join(os.tmpdir(), 'build-vue-m0-'));
  try {
    ...
    assert.match(result.stdout, /m0: 12 cells/);
    ...
    for (const id of ids) {
      for (const rows of [0, 2000]) {
        assert.equal(
          fs.existsSync(path.join(out, id, `rows-${rows}/main.lynx.bundle`)),
          true,
        );
      }
    }
    assert.match(
      fs.readFileSync(
        path.join(out, 'reactlynx-0-126-default/rows-0/main.web.bundle'),
        'utf8',
      ),
      /ui-react:0:0:0:off:production:react-rspeedy/,
    );
    assert.match(
      fs.readFileSync(path.join(out, 'reactlynx-0-126-et/rows-0/main.web.bundle'), 'utf8'),
      /ui-react:0:0:1:et:production:react-rspeedy/,
    );
    assert.match(
      fs.readFileSync(
        path.join(out, 'vue-lynx-0-5-vdom-ifr-et/rows-2000/main.web.bundle'),
        'utf8',
      ),
      /ui-vdom:2000:1:1:ifr-et:production:vue-rspeedy/,
    );
  } finally {
